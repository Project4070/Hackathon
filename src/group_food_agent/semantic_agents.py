...
MENU_ENRICHMENT_PROMPT_VERSION = "menu-enrichment-v1"
...
class MenuSemanticCandidateV1(ContractModel):
    ...
    category_code: Literal["chicken", "pizza", "unknown"]
...
You may normalize names, classify only chicken/pizza/unknown, identify variants,
and classify vegetarian/spice semantics. Preserve any serving or vegetarian
...
class MenuSemanticEnricher:
    """Bounded live runner with reusable hash/model/prompt cache."""
    ...
    def __init__(
        ...
        *,
        ...
        self._cache: dict[tuple[str, str, str], MenuSemanticCandidateV1] = {}
    ...
    async def enrich(self, source_url: str, visible_text: str) -> tuple[MenuSemanticCandidateV1, bool]:
        ...
        digest = source_content_hash(sanitized)
        cache_key = (digest, self.model, MENU_ENRICHMENT_PROMPT_VERSION)
        if cache_key in self._cache:
            return self._cache[cache_key], True
        if not self._runner_was_injected and not os.getenv("OPENAI_API_KEY"):
            raise RuntimeError("OPENAI_API_KEY is required for uncached menu semantic enrichment")
        prompt = json.dumps(
            {
...
def build_menu_enrichment_agent_tool(model: str | None = None):
    """Return the focused SDK agent as a tool for a future async cache refresher."""

    return build_menu_semantic_agent(model).as_tool(
        tool_name="menu_semantic_enrichment_agent",
        tool_description=(
            "Normalize one sanitized uncached menu record. Never supplies prices, "
